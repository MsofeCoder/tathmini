...
- [x] Pending-sync badge and queue view — now the **Reports** tab (`apps/web/src/app/reports/page.tsx`), carrying the waiting count on its label (`Reports · N`, the prototype's `Pending · N` treatment) and a top tab bar with the three states a finished assessment can be in: **Drafted** (held back on purpose), **Submitted** (reached the College), **Pending** (queued marks and queued report sends, listed by trainee, instrument, when it was marked and any failed attempts). Each trainee appears in exactly one, ranked pending > drafted > submitted (`lib/reports-tabs.ts`, unit-tested). Every row offers a preview, which needs a connection and says so. `/pending` redirects here for phones whose service worker precached it; the offline screen carries the same queue list (PR #21/#22). All three lists are read from IndexedDB on the client, because the reason something is queued is that the network failed — a screen needing the network to describe its own backlog would be empty exactly when it matters. The **Moves** tab was removed from the bar at the same time: it was inert, and a dead quarter of the bar reads as broken rather than as unbuilt (see `MEMORY.md` 2026-09-06)
