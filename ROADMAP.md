...
- [~] Dexie outbox: idempotent UUID keys, exponential backoff, replay on reconnect — `apps/web/src/lib/outbox.ts` + `apps/web/src/app/outbox-drainer.tsx`, keyed per (trainee, instrument) with the database's own unique index as the real idempotency backstop, replayed on `online`/focus. **2026-09-06:** the drainer now gates on a real reachability probe rather than `navigator.onLine` (which is true on a wifi that routes nowhere, so every entry failed and each failure pushed the backoff further out), and each entry records the supervisor who queued it, so a shared phone never replays one person's marks under another's session. **Exponential backoff now built** (10s → 20s → 40s, capped at 5 min, ±20% jitter, PR #27): without it the drainer retried every entry on every `online` event, and in flapping signal a submission failing for a reason no retry can fix was re-sent on every flap. Still **not** Background Sync — deliberate, see `MEMORY.md`. Stays `[~]` for one reason only: **the exit gate has never been proven in a real browser** **2026-09-07: replay on reconnect is deliberately GONE.** At the user's explicit instruction (asked and reaffirmed — see `MEMORY.md` 2026-09-07), nothing leaves the phone that a supervisor did not press a button for: `OutboxDrainer` is deleted and the same pass now runs from the **Send** button on the Reports screen (`lib/send-pending.ts`). Every send control is gated on live reachability — online offers Save draft AND Submit, offline offers Save draft only. Manual Send ignores backoff (`listSendable()`); the shared-phone owner check stays. **This retires the exit gate below**: reconnecting now produces NO submissions, by design. `drainOutbox`'s own invariant is untouched — an entry is removed only after the server confirms, `already_submitted` counts as confirmation, marks drain before reports.
...
- [x] Pending-sync badge and queue view — now the **Reports** screen (`apps/web/src/components/screens/reports-screen.tsx`), carrying the waiting count on its bottom-nav label (`Reports · N`, the prototype's `Pending · N` treatment) and a top tab bar with the three states a finished assessment can be in: **Drafted** (held back on purpose), **Submitted** (marks reached the College, and the report with them if it has been sent) and **Pending** (queued marks and queued report sends, listed by trainee, instrument, when it was marked and any failed attempts). Each trainee appears in exactly one, ranked pending > drafted > submitted (`lib/local/reports.ts`, unit-tested). Every row offers a preview, which needs a connection and says so. `/pending` still serves this screen for phones that have that url precached or bookmarked. All three lists are read from the device through `liveQuery`, because the reason something is queued is that the network failed — a screen needing the network to describe its own backlog would be empty exactly when it matters. The **Moves** tab was removed from the bar at the same time: it was inert, and a dead quarter of the bar reads as broken rather than as unbuilt (see `MEMORY.md` 2026-09-06).

**Exit gate (SUPERSEDED 2026-09-07):** a complete assessment survives airplane
mode, a force-quit and a reboot; reconnecting produces exactly one submission,
never two.

The first half still holds and still matters — drafts survive airplane mode, a
force-quit and a reboot. The second half no longer describes the app: at the
user's explicit instruction nothing replays on reconnect, so a supervisor must
open Reports and tap **Send** themselves. The replacement gate is: *a complete
assessment survives airplane mode, a force-quit and a reboot; pressing Send
once back in coverage produces exactly one submission, never two, and the app
never offers Submit when it cannot send.* **This needs a field instruction to
supervisors, not just code** — see `MEMORY.md` 2026-09-07.
